...
## ✅ Tuya Cloud quota blocker: resolved (2026-09-09)

For a while, device control was down in production because Tuya Cloud's "IoT Core" quota (and a separate, one-time-only "Cloud Develop Base Resource Trial" pool underneath it) was exhausted, which also blocked `tinytuya wizard`'s cloud-based `local_key` fetch — the exact tool `bridge/README.md` originally documented for setup. **Fixed without ever needing that quota**: used [`vineetchoudhary/tuya-local-key`](https://github.com/vineetchoudhary/tuya-local-key) (QR-code login via the same official flow Home Assistant's Tuya integration uses — no developer account, no Access ID/Secret) to get every device's `local_key` + DP index→code mapping, then a pure local network scan (`tinytuya.deviceScan()`, no cloud) for each device's LAN `ip`/`version`. Full steps are in `bridge/README.md`'s "One-time setup" section — that's now the primary documented path; the plain `tinytuya wizard` route stays as a fallback for whoever has working IoT Core quota.

Also found and fixed against real hardware: `set_multiple_values()` fails ("Unexpected Payload from Device") on at least the v3.3 lamp; `send_commands()` in `bridge.py` now loops individual `set_value()` calls instead, verified working on the lamp (v3.3) and all 4 plugs (v3.4).
...
**Status as of this note**: `bridge/` is verified working locally (Docker, all 5 devices, real commands confirmed against physical hardware). Not yet done: the Cloudflare Tunnel, and setting `BRIDGE_URL`/`BRIDGE_TOKEN` as Worker secrets + deploying `src/worker.js`. Check `git log`/production status before assuming this is still the state — this note should be deleted once the tunnel+deploy step lands.
...
| _current_ | Got the bridge fully working against real hardware, without ever needing a working Tuya Cloud quota: `local_key`/DP mapping via `tuya-local-key`'s QR-login flow, LAN `ip`/`version` via a local `tinytuya.deviceScan()`. Fixed `send_commands()` in `bridge.py` (`set_multiple_values` failed on the lamp; loops `set_value` now, verified on all 5 devices). Still open: the lamp's `colour_data` DP is raw hex over local protocol, not the JSON shape `light-model.js` expects. See "Tuya Cloud quota blocker: resolved" near the top of this document. |
