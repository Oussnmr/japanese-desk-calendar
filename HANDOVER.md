...
- **Performance fix** (RGB/plug commands felt slow at first): `bridge.py` was opening a brand-new local-protocol connection (TCP + the Tuya session handshake) *per request*, and a single colour change did several sequential device round-trips (read current HSV, send, then the Worker's own confirm-by-polling). Two changes, both verified against real hardware: (1) `tuya_device()` now caches one persistent `tinytuya.Device` per device id (`set_socketPersistent(True)`), reused across requests, with a one-time reconnect-and-retry if a cached connection goes stale — cut repeat status reads from ~200ms to ~70-80ms locally. (2) `send_commands()` sends with `nowait=True` instead of `nowait=False` — waiting for the device's own low-level command ACK measured 235ms-1000ms and varies a lot, and is redundant with the Worker's own `/status` re-poll that already confirms the change; cut writes to ~90-170ms locally (~260ms through the tunnel, which adds its own round trip). The device still applies the command correctly either way - only the synchronous low-level ACK wait was removed.
...
| _current_ | Cloudflare Tunnel created (`bridge.nemri.uk`, a small dedicated domain bought via Cloudflare Registrar specifically to avoid touching the owner's e-commerce domain's DNS), Worker secrets `BRIDGE_URL`/`BRIDGE_TOKEN` set and confirmed live, `TUYA_API_KEY`/`SECRET`/`REGION` deleted from the Worker (confirmed unused). Migration off Tuya Cloud is complete end-to-end. See "Tuya Cloud migration: complete and live" near the top of this document. |
