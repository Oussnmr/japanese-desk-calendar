...
## 🚧 Current status: mid-migration, device control is down (2026-09-09)

**Read this before touching Tuya/bridge anything.** `src/worker.js` on `main` now expects a local bridge (`BRIDGE_URL`/`BRIDGE_TOKEN` secrets, §2/§5) instead of Tuya Cloud, but **that bridge is not running anywhere yet and those secrets are not set** — so `/api/light/*` and `/api/plug/*` currently answer `503` in production. This is not a regression to "fix" by reverting; it's the middle of a deliberate migration (see the change history entry for commit `13752f2` and `bridge/README.md`). Do not restore the old Tuya-Cloud-signing code in `src/worker.js` without checking with the owner first.

**Why**: Tuya Cloud's free "IoT Core" API quota ran out. Worse, its underlying "Cloud Develop Base Resource Trial" billing pool ($0.2 USD, resets monthly) is *also* exhausted and is a **separate, one-time-only** free tier — re-subscribing to it fails ("You have subscribed to the trial edition before, and cannot subscribe again"). Extending IoT Core's own service validity (which support can approve quickly, and did) does **not** refresh this separate resource pool.

**What this blocks**: `bridge/README.md`'s one-time setup (`python -m tinytuya wizard`, to fetch each device's `local_key`) itself needs a working Tuya Cloud API call, and currently fails with the same `Code 28841004: 'IoT Core trial quota is exhausted.'` — so we can't finish the bridge setup either. Chicken-and-egg: the migration away from Tuya Cloud needs one working Tuya Cloud call to bootstrap.

**Things tried and ruled out** (don't re-attempt without new information):
- Re-subscribing to the IoT Core Trial Edition product a second time — Tuya blocks this (one-time only).
- `tuya-cli`/TuyAPI CLI's "Add App Account" QR-login flow — still requires the same Cloud Developer project and almost certainly the same quota-gated API underneath (per its own docs, it's an addition *within* a dev project, not an alternative to one).
- Extracting `local_key` from the Tuya Smart consumer app's on-disk storage: set up an Android Studio emulator (AVD "Medium Phone", **Google APIs** x86_64 image — not Google Play, specifically so `adb root` works), sideloaded Smart Life/Tuya Smart (`com.tuya.smart`) via `adb install-multiple` (APKMirror `.apkm` bundles need extracting `base.apk` + the matching arch split first — no x86_64 split exists, relies on the emulator's ARM translation), logged in, confirmed all 5 devices show and control correctly from the app. Then searched the entire `/data/data/com.tuya.smart/` tree (root shell) for `local_key`/`localKey` and both known device ids: found the 5 devices' `id`/`name`/`productId` in `files/energy_card_device_list/*.json`, but **no `local_key` anywhere in persistent storage** — the app appears to never write it to disk, likely only holding it in memory when needed. Filesystem extraction is a dead end; the next step up would be Frida-based runtime memory interception (hook the method that receives `localKey` from the server), not attempted — flagged as high-effort/uncertain (obfuscation, possible anti-tampering) versus just waiting.
- Paid tiers: "Flagship Edition (one year)" priced at **€25,000** — absurd for 5 personal devices, ruled out. Cheaper tiers (Starter/Trial-successor) not fully checked; worth a look before assuming payment is off the table.

**Realistic remaining options**, in the owner's stated preference order: (1) wait for the free monthly resource-pool reset (next: 2026-10-01), (2) attempt Frida-based `local_key` interception, (3) check pricing on a cheaper paid IoT Core tier than Flagship. No option has been executed yet.

**If you're picking this up cold**: check `git log` for how recent this note is before trusting it — the owner may have resolved this since. If still blocked, don't redo the ruled-out attempts above; ask the owner which of the three remaining options they want, or check IoT Core's status on the Tuya console yourself before assuming nothing has changed.
...
| `13752f2` | Tuya Cloud's free "IoT Core" quota was exhausted, breaking every device button at once. Rather than just renewing it, moved device control off Tuya Cloud entirely: `bridge/` (renamed from `tools/lepro-light/`) now controls all 5 devices over the Tuya **local LAN protocol** on the owner's home PC, reached by the Worker through a Cloudflare Tunnel. `src/worker.js`'s `deviceRequest()` now proxies to the bridge (`BRIDGE_URL`/`BRIDGE_TOKEN` secrets) instead of signing Tuya Cloud requests; `TUYA_API_REGION/KEY/SECRET` are no longer Worker secrets. See §2, §5, §11, and `bridge/README.md`. **Not yet deployed/finished** — see the "Current status" callout at the top of this document. |
