...
**Local RGB payload support is complete (2026-09-09)**: the lamp's `colour_data` DP is a legacy Type A `rrggbbhhhhssvv` hex string (14 digits), not the Tuya Cloud JSON shape. `light-model.js` now detects and round-trips that format, the newer 12-digit `hhhhssssvvvv` format, and JSON. For Type A status it derives HSV from the leading RGB bytes because this lamp has returned stale/inconsistent trailing hue bytes while in white mode. The Worker also confirms the requested HSV values instead of accepting colour mode alone. Real-hardware verification successfully applied and read back red, green, and blue, then restored CHILL (35% / 50%).
